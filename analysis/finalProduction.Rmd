...
schoolBody <- cleanBans %>% 
  filter(School.Name %in% body$School.Name) %>% 
  filter(type == "body",
         item %in% bodyButtons) %>% 
  spread("item", "type", fill = NA) %>% 
...
wordsToExtract <- c("distract", "disrupt", "appropriate", "safe", "health", "interfere", "clean", "modest", "neat", "respect")
...
shuffled <- unlistExtracted[sample(nrow(unlistExtracted)),]
